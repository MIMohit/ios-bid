...
export const Route = createFileRoute("/category/$slug/today")({
  ...
  loader: async ({ context, deps, params }) => {
    ...
    const [board] = await Promise.all([
      ...
      context.queryClient.ensureQueryData(
        convexQuery(api.board.podium, { window: "all", categorySlug: params.slug }),
      ),
    ]);
...
function CategoryToday() {
  ...
  const podium = useSuspenseQuery(
    convexQuery(api.board.podium, { window: "all", categorySlug: slug }),
  ).data;
      ...
      <main>
        {top ? (
          <SpotlightPanel row={top} clicks={clicks[top.id] ?? 0}>
            <BidBar priceForTop={board.priceForTop} window="today" categorySlug={slug} />
          </SpotlightPanel>
        ) : (
          <BidBar priceForTop={board.priceForTop} window="today" categorySlug={slug} flat />
        )}
        ...
        <div className="page shell">
          ...
          <div>
            <Board
              ...
              activity={activity}
              podium={{
                heading: `All-time top ${name}`,
                href: path,
                empty: `No ${name} app has been paid for yet.`,
                rows: podium,
              }}
              heading={`${name} today`}
              caption={
