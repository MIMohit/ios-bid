...
/** Every slug, as a literal union. Adding a category without mapping it fails typecheck. */
...
export function categorySlugFor(genreId?: string | number, genreName?: string): string {
  const byId = genreId == null ? undefined : BY_GENRE_ID[String(genreId)];
  if (byId) return byId;
  const byName = genreName ? BY_NAME.get(genreName.toLowerCase()) : undefined;
  return byName ?? "other";
}
